...
def main() -> int:
    ...
    ap.add_argument(
        ...
        choices=["local"],
        default="local",
        help="Force Whisper backend. Default: local (faster-whisper, no API key needed).",
    )
    args = ap.parse_args()
    ...
    print(f"[ec-watch] working dir: {work}", file=sys.stderr)

    print(
        "[ec-watch] downloading via yt-dlp…" if is_url(args.source) else "[ec-watch] using local file…",
        file=sys.stderr,
    )
    ...
    print(f"[ec-watch] extracting ~{target} frames at {fps:.3f} fps over {scope}…", file=sys.stderr)
    ...
    if dl.get("subtitle_path"):
        try:
            ...
            print(f"[ec-watch] subtitle parse failed: {exc}", file=sys.stderr)
    ...
    if not transcript_segments and not args.no_whisper:
        ...
        if backend:
            try:
                ...
                print(f"[ec-watch] whisper fallback failed: {exc}", file=sys.stderr)
        else:
            hint = (
            ...
            print(
                f"[ec-watch] {hint} — run `python3 {setup_py}` to enable the Whisper fallback",
                file=sys.stderr,
            )

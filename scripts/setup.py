...
- Silent on success: --check exits 0 with no output when everything's ready so
  that /ec-watch doesn't spam "setup is complete" on every turn.
- Idempotent: re-running the installer is safe — it never clobbers existing
  keys and only appends missing ones.
- SETUP_COMPLETE=true in ~/.config/ec-watch/.env tells us the user has been
...
CONFIG_DIR = Path.home() / ".config" / "ec-watch"
CONFIG_FILE = CONFIG_DIR / ".env"
ENV_TEMPLATE = """# /ec-watch API configuration
#
# Whisper transcription fallback — used only when yt-dlp cannot get captions
# (or when you point /ec-watch at a local file with no subtitles).
#
...
#
# Leave both blank to disable Whisper — /ec-watch will still work, but videos
...
def _check_file_permissions(path: Path) -> None:
    """Warn to stderr if a secrets file is world/group readable (Unix only)."""
    ...
    try:
        ...
        if mode & 0o044:
            sys.stderr.write(
                f"[ec-watch] WARNING: {path} is readable by other users. "
...
def _have_api_key() -> tuple[bool, str | None]:
    # ec-watch uses local faster-whisper — SETUP_COMPLETE=true means ready, no API key needed
    if _read_env_key("SETUP_COMPLETE") == "true":
        return True, "local"
...
def _scaffold_env() -> bool:
    """Create ~/.config/ec-watch/.env with placeholders if missing."""
...
def cmd_check() -> int:
    ...
    if not s["has_api_key"]:
        parts.append("setup incomplete (run installer)")
    installer = Path(__file__).resolve()
    sys.stderr.write(
        f"[ec-watch] setup needed ({'; '.join(parts)}). "
...
def cmd_install() -> int:
    ...
    if has_key:
        ...
        # ec-watch uses local whisper — binaries alone mean we're ready
        _write_setup_complete()
        print("[setup] ready. whisper backend: local (faster-whisper, no API key needed)")
    if installed_deps:
        print("[setup] installed dependencies; /ec-watch is fully set up.")
    return 0

#!/usr/bin/env bash
# SessionStart hook for /ec-watch — one-line status so users know what's wired up.
# Silent on ready state to avoid spam. Points at the installer when something
# is missing.
set -euo pipefail

CONFIG_FILE="$HOME/.config/ec-watch/.env"
...
if [[ -f "$CONFIG_FILE" ]]; then
  ...
  if [[ -n "$perms" && "$perms" != "600" && "$perms" != "400" ]]; then
    echo "/ec-watch: WARNING — $CONFIG_FILE has permissions $perms (should be 600)."
...
if [[ -z "$HAS_FFMPEG" || -z "$HAS_YTDLP" ]]; then
  echo "/ec-watch: needs ffmpeg + yt-dlp. Run \`python3 \$CLAUDE_PLUGIN_ROOT/scripts/setup.py\` once to install and scaffold config."
elif [[ -z "$HAS_GROQ" && -z "$HAS_OPENAI" ]]; then
  echo "/ec-watch: ready for videos with native captions. Add GROQ_API_KEY (preferred) or OPENAI_API_KEY to ~/.config/ec-watch/.env to unlock Whisper fallback."
else
  echo "/ec-watch: ready."
fi
